...
$(".dragit").each( function(i){
	iElPos[i]=$(this).offset();
	iElPosX[i]=iElPos[i].left;
	iElPosY[i]=iElPos[i].top;
});
...
$(".dragit").mousedown(function(e){
	inZ=inZ+1
   	...
   	var curPosX=curPos.left;
   	var curPosY=curPos.top;
   	difX=e.pageX-curPosX;
   	difY=e.pageY-curPosY;
	
	elEq=$(".dragit").index(this);
	$(this).clone().css({left: "0", top: "0", position: "relative"}).appendTo("#dragHelper").end().hide();
	$("#dragHelper").css({left: curPosX, top: curPosY, zIndex: inZ, opacity: "0.80"})
    ...
    $("body").bind("mousemove",function(e){
	 if($("#dragHelper div").length ){
		$("body").addClass("noselect");
  		...
  		if(toTarget==1){ 
		 $(".target").eq(hit).addClass("act_targ");   
  		}else{
		 $(".target").removeClass("act_targ");
		 $("body").addClass("noselect");
		}
...
function draggerStop(elEq,difX,difY){
	$("body").one("mouseup", function(e){
		...
		if(toTarget==1){
			if($.browser.msie){
				$("#dragHelper").css({zIndex: inZ, left: targX[hit], top: targY[hit]});
			}else{
				$("#dragHelper").animate({zIndex: inZ, left: targX[hit], top: targY[hit]},"fast");
			}
	   		$("#dragHelper").queue(function() {
	   	   		$(".dragit").eq(elEq).css({zIndex: inZ, left: targX[hit], top: targY[hit]}).show();
	   			$("#dragHelper").replaceWith("<div id=\"dragHelper\" style=\"position: absolute\"><\/div>");
	   		});
		}else{
			if($.browser.msie){
				$("#dragHelper").css({zIndex: inZ, left: iElPosX[elEq], top: iElPosY[elEq]});
			}else{
				$("#dragHelper").animate({zIndex: inZ, left: iElPosX[elEq], top: iElPosY[elEq]},"fast");
			}

	   		$("#dragHelper").queue(function() {
	   	  		$(".dragit").eq(elEq).css({zIndex: inZ, left: iElPosX[elEq], top: iElPosY[elEq]}).show();
	   	   		$("#dragHelper").replaceWith("<div id=\"dragHelper\" style=\"position: absolute\"><\/div>");
			});
		}
