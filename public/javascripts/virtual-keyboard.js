...
function dragKeyboard(item, panel) {
	$(".keys_wrapper").append('<div class="draggerClone" ><\/div>');
	panel.mousedown(function(e) {
		...
		$(".toDrag").clone().css({position: "relative", left: '0', top: '0'}).appendTo(".draggerClone").end().css({visibility: 'hidden'});
...
function insertFromKeyboard(string) {
	...
	var activeEl = $(".active_input").get(0);
	var t = activeEl.value,  s = caretPosition; 
...
function keyBoardUnfix() {
  if (itsFixed) {
	...
	//var tLeftMarg = $(".keys_poser").css('margin-left');
