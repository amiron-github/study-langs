	...
	def get_cat_ex(lang)
	 ...
	 test_categories.each do |tc|
		...
		if category_id != nil
			category = Category.find(category_id)
			if category.lang == lang
			unless category.tag == 'everyday_course' || category.tag =='beginner_course' || category.tag =='phonetics_course'||category.tag =='grammar_course'||category.tag =='reading_course'||category.tag =='kids_lessons_en'
